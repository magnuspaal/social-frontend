...
export default function ChatBubble({message, chat}: {message: ChatMessage, chat: Chat}) {
  ...
  return (
    <div className={`flex flex-col my-[2px] ${isMe ? 'right' : 'left'}`}>
      <div
        style={{wordBreak: "break-word", background: `${isMe ? myColor : theirColor}`, color: `${isMe ? myTextColor : theirTextColor}`}} 
        className={`flex max-w-[60%] text-sm rounded px-4 py-2 mx-2 ${isMe ? `place-self-end bg-secondary` : 'place-self-start bg-shade'}`}>
          {message.content}
      </div>
    </div>
  )
