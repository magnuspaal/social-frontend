...
export default function ChatSettingsPage({chat}: {chat: Chat}) {
  ...
  const updateChatSettings = async () => {
    ...
    await clientMessagingService.updateChatSettings(chat.id, JSON.stringify({chatSettings}))
      ...
      <div className='mx-5 flex flex-col items-center gap-2'>
        ...
        <div className='flex flex-col bg-white divide-y rounded-md max-w-[400px] w-full px-1'>
          ...
          <ChatSettingsColor 
            ...
            message={`${!theirContrast ? t("chat.settings.text_readability") : ''}`}
            openUp={true}
          />
          <ChatSettingsColor 
            description={t('chat.settings.colors.background_color')} 
            color={backgroundColor} setColor={setBackgroundColor} 
            message={`${!textContrast ? t("chat.settings.text_readability") : ''}`}
            openUp={true}/>
          <ChatSettingsColor 
            description={t('chat.settings.colors.element_color')} 
            color={elementColor} setColor={setElementColor} 
            message={`${!elementContrast ? t("chat.settings.colors.element_readability") : ''} ${!elementTextContrast ? t("chat.settings.element_text_readability") : ''}`} 
            openUp={true}
          />
        </div>
        <button 
